...
const ProgrammingSupServiceProvider = (props) => {
  ...
  async function getProgrammingSupService() {
    try {
      const res = await axios.get("/sup/programming");
      setService(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  return <ProgrammingSupServiceContext.Provider value={state}>{props.children}</ProgrammingSupServiceContext.Provider>;
};
